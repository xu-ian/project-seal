...
searchRoutes.route('/listcompanies').get((req, res) => {
  ...
  Company.find({}, {company_title:1, tagline:1, logo:1, _id:1}).then(companies => {
    res.json(companies);
...
searchRoutes.route('/listcompanies/:name').get((req, res) => {
  ...
  Company.find({company_title : name}, {company_title:1, tagline:1, logo:1, _id:1}, {limit: 15}).then(companies => {
    res.json(companies);
...
searchRoutes.route('/listusers').get((req, res) => {
  // Get 15 companies information 
  User.find({}, {username:1, userbio:1, email:1, _id:1}).then(users => {
    res.json(users);
...
searchRoutes.route('/listusers/:name').get((req, res) => {
  let name = new RegExp(req.params.name, 'i');
  // Get maximum 15 companies which company_title contains "name" as substring.
  User.find({username : name}, {username:1, userbio:1, email:1, _id:1}, {limit: 15}).then(users => {
    res.json(users);
